...
public class ExtractedPostApplicationServiceImpl implements ExtractedPostApplicationService {
    ...
    public Optional<DisplayExtractedPostDto> findById(Long id) {
        ExtractedPost extractedPost= extractedPostService.findById(id).orElseThrow(()-> new PostNotFoundException(id));
        return Optional.of(DisplayExtractedPostDto.from(extractedPost));
    }

    @Override
    public Optional<DisplayExtractedPostDto> deleteById(Long id) {
        ExtractedPost extractedPost= extractedPostService.deleteById(id).orElseThrow(()-> new PostNotFoundException(id));
        return Optional.of(DisplayExtractedPostDto.from(extractedPost));
    }
}
